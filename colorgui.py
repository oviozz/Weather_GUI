...
def getWeather(window):
    try:
        ...
        label.config(text=location,foreground="Green")
        label0.config(text=time,foreground="Red")
        label1.config(text=temp + ' °F',foreground="Dark Blue")
        label2.config(text=x[:-7],foreground="Blue")

        # Buttons function / placement
...
weather_title.config(font=t,foreground="Black")
...
by.config(font=font_size,foreground="Orange")
by.pack()
...
# Accept input from the textfieldk
textfield.focus()
